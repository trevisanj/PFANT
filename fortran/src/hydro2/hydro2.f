command-line option: ZPH (default=12) because it will no longer be taken from file
...
!> @todo tidy up absoru.f90 before proceeding with these dimensions here
        ...
        COMMON/LECT1/ABMET,ABHEL /TITIOLINE/TITRE(5)
C
...
C actually this comment is relevant -----> ............adios!C       (si modeles_tit va bien modeles_tiabs du modele = TITRE de absoru)
C ............adios!C
...
C
      ...
      CALL READER5N(NH,TETA,CLOG,PG,TO,modeles_ntot)
...
C
            IF(ECRIT)   THEN
                ...
                WRITE (6,201)
                WRITE (6,202) (I,NH(I),TETA(I),CLOG(I),TO(I),I=1,modeles_ntot)
                WRITE(6,*)' '
                   if(papier) then
                   write(11,47)ABMET,(modeles_tit(I),I=1,5)
                   write(11,201)
                   write(11,202)
     1                (I,NH(I),TETA(I),CLOG(I),TO(I),I=1,modeles_ntot)
                   write(11,*)' '
                   end if
            END IF
C
...
C
      ...
      CALL OPTIC1(KC,NH,modeles_ntot,TAUC)
