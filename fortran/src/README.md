...
documentation. The documentation is written within code, and is thus relatively 
easy to keep up to date. (https://en.wikipedia.org/wiki/Doxygen)

To be honest, the main reason why Doxygen was adopted is to impose discipline to
commenting the code. I personally find the routine call graphs generated by Doxygen
to be cool, but I prefer to read the documentation in the source code itself.

This section describes how to compile and deploy HTML code using Doxygen and contains some workarounds
to some Doxygen bugs.
...
```shell
deploy_doxy-fortran.sh</code> inside directory <code>PFANT
```

#### Troubleshooting doxygen compilation
 
##### Segmentation fault running doxygen
...
except for `parameter`s which are all *UPPERCASE*.
Constants not declared as `parameter` (but constant in their *meaning*) may be also in uppercase.

The main reason for this was a limitation in Doxygen, which converts all symbols
