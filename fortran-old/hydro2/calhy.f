...
30    RES=2.*RES
 ...
 1017 CONTINUE
      ...
      SUBROUTINE FLUXIS (T1,T2,B,NTOT,PTDISK,MU,JM,F,FC,IOP)
...
C     ON ADMET UNE EXTRAPOLATION JUSQU A 1/10 DE X2-X1 ET XN-X(N-1)
      DIMENSION  X(N),Y(N),TT(ITOT),FTT(ITOT)
      INV = -1
      IF(X(N).LT.X(1) ) INV=1
C     DXA ET DXZ  : LIMITES SUPPORTABLES D EXTRAPOLATION
      DXA=(X(2)-X(1))/10
      DXZ=(X(N)-X(N-1))/10
      XX1=X(1)-DXA
      XXN=X(N)+DXZ
      ...
      T=TT(K)
      IF (INV)5,6,6
5     IF( (T.LT.XX1) .OR. (T.GT.XXN) )   GO TO 10
      DO 1 J=1,N
      I=J
      IF(T-X(J))3,2,1
1     CONTINUE
      GO TO 3
6     IF( (T.GT.XX1) .OR. (T.LT.XXN) )   GO TO 10
      DO 7 J=1,N
      I=J
      IF(T-X(J) )7,2,3
7     CONTINUE
...
c     Nouvelle subroutine NAITK3 remplace AITK3 et AITK30
      ...
      SUBROUTINE OPTIC1(KAP,NH,NTOT,TO)
...
12    SOM=SOM+S
      ...
      SUBROUTINE RAIEHU(LL,NA,NB,NBMIN,NBMAX,CLAM,C1,C,DLAM,L,J1,IND,KQ)
C
...
C     KQ=1, CALCUL D UN PROFIL PUREMEMT QUASISTATIQUE POUR IONS + ELETRO
C
      LOGICAL AMORES,STARK
      REAL*4 L,LIM,LAC,LV
      REAL*8 CLAM,XDP(50),XBDP
      DIMENSION F1(220),VAR(220),V(300),PHI(300),RESC(50)
      DIMENSION BIDON(50,99)
      DIMENSION L(50,99),C(20),DLAM(50),     AS(150),ALFA(50),LV(50),AL(
...
20    DO 1010 J=1,JMAX
      ...
      S1=CT/ALFA(J)**2.5
      IF ((DLAM(J).EQ.0.).OR.(KQ.EQ.1)) GO TO 23
      RAC=DLAM(J)**0.5
...
1013  AL(L1)=ALOG10(LV(J))
      IF(.NOT.AMORES) GO TO 4013
4000  CALL PRONOR(JMAX,IQM,IJ,LV,ALFA,FAC,AX)
      DO 4006 J=1,JMAX
      LV(J)=LV(J)*AX
4006  BIDON(J,I)=LV(J)
...
200   U(J)=TAB(MM,NC)+S0*MU+PHI*(NU-MU)
     ...
     2+(BB/(2.*ACA*EN))*(PI-ATAN((2.*RCA+RM)/EN)-ATAN((2.*RCA-RM)/EN)))
      ...
      FUNCTION FT(T,N,X,F)
CINTERPOLATION D UNE LISTE A PAS NON CONSTANT
      DIMENSION X(N),F(N)
      DO 1 J=1,N
      I=J
      IF(T-X(J))3,2,1
