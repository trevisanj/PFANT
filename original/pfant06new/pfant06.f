...
C	COMMON AVEC LE SP READER ET LE SP DE TRACE
	COMMON /COM6/TEFF,GLOG,ASALALF,NHE,TIABS(5),TIT(5)  
C old	COMMON/COM6/TETAEF,GLOG,ASASOL,NHE,TIT(5)
	common/alog/asalog
...
C	
	...
	CALL TURBUL(INTERP,IVTOT,TOLV,VVT,NTOT,T5L,VT)	
		...
		ELSE
		WRITE(6,132)
		WRITE(6,133) ((TOLV(I),VVT(I)),I=1,IVTOT)
		END IF
C  *****************************************************************
C				III
...
C
	READ(4,1500) FILEFLUX
        OPEN(UNIT=17,FILE=FILEFLUX,ACCESS='DIRECT',STATUS='NEW',
	1 RECORDSIZE=6025,ASSOCIATEVARIABLE=IK)
	IK=1
...
C	WRITE(6,160)  ((TTD(D),FL(D) ),D=1,DTOT)
	...
	WRITE(6,122)
	   if(nblend.ne.0) then
		DO K=1,NBLEND
		WRITE(6,121) ELEM(K),IONI(K),LAMBDA(K),KIEX(K),ALGF(K),
	1	alog10(ABOND(K)),CH(K),GR(K),GE(K),ZINF(K),CORCH(K)
	WRITE(6,123)
		END DO
	  end if
	END IF
...
c       if(aaa.gt.ikeytot) ikeytot=ikeytot+1
        ...
        L0=LLZERO-10.
        LF=LLFIN+10.
        WRITE(17'IK) IKEYtot,(TIT(I),I=1,5),TETAEF,GLOG,ASALOG,NHE,AMG,
...
C	CE S.P. LIT SUR DISQUE ACCES DIRECT NH,TETA,PE,PG,T5L,NTOT
	...
	REAL NH,NHE
	COMMON /COM6/TEFF,GLOG,ASALALF,NHE,TIABS(5),TIT(5)    
C	COMMON /COM6/TETAEF,GLOG,ASASOL,NHE,TIT(5)
	COMMON/ALOG/ASALOG
	DATA   BLC/'    '/
	DO 7 I=1,5
7	TIT(I)=BLC
	...
	OPEN(UNIT=18,ACCESS='DIRECT',TYPE='OLD',
     1    name='modeles.mod',
	1 RECORDSIZE=300,ASSOCIATEVARIABLE=ID)
	ID=1
	IDEF=211939
...
9	READ(18'ID) NTOT,DETEF,DGLOG,DSALOG,ASALALF,NHE,TIT,TITABS
