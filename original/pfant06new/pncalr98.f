...
135	FORMAT(5(I4,F7.3,F5.2))
	...
	COMMON/COM1/ EELEM(40),ELMAX,LAMBDA(5),LMAX,KKIEX(4),KIMAX,LECS
	COMMON /COM6/TETAEF,GLOG,ASASOL,NHE,TITRE
...
C	*** LA FONCTION EST ENSUITE NORMALISEE
	...
	1 YN(15)/3*.6,.5,2*.4,4*.3,1.,.9,.8,2*.7/,D0(25),D1(25),D2(25)
	2,D3(25),D4(25),HN(25),H/.201/,XX(3)/.5246476,1.65068,.7071068/
	3,HH(3)/.2562121,.2588268E-1,.2820948/,NBY2(19)/9.5,9.,8.5,8.,
	47.5,7.,6.5,6.,5.5,5.,4.5,4.,3.5,3.,2.5,2.,1.5,1.,.5/,C(21)/
	5.7093602E-7,-.2518434E-6,.8566874E-6,-.2787638E-5,.866074E-5,
	6-.2565551E-4,.7228775E-4,-.1933631E-3,.4899520E-3,-.1173267E-2,
	7.2648762E-2,-.5623190E-2, .1119601E-1,-.2084976E-1,.3621573E-1,
	8-.5851412E-1,.8770816E-1, -.121664,.15584,-.184,.2/
	LOGICAL TRU/.FALSE./
	B(1)=0.
	B(2)=0.7093602E-7
	IF (TRU) GO TO 104
...
7	TIT(I)=BLC
	...
	OPEN(UNIT=18,ACCESS='DIRECT',TYPE='OLD',
     1   name='modeles.dat',
	1 RECORDSIZE=256,ASSOCIATEVARIABLE=ID)
	ID=1
	IDEF=211939
...
9	READ(18'ID) NTOT,DETAEF,DGLOG,DSALOG,DNHE,DAMET,TIT
...
5	IF(DDIF.GT.0.0005)   GO TO 9
	ASASOL=10.**ASALOG
	ID=ID-1
	READ(18'ID)BID,((NH(I),TETA(I),PE(I),PG(I),T5L(I)),I=1,NTOT)
	write(6,*)'            NH           TETA              PE',
	1 '             PG     To(5000)'
	DO 12 I=1,5
...
C
	IF(GRAPH) TYPE *,' PATIENCE... JE CALCULE...'
		DO K=1,NBLEND
		ECAR(K)=ECART(K)
		END DO
c
